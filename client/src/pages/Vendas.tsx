...
export default function Vendas() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) => db.createVenda(data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/vendas"] }); toast({ title: "Venda criada!" }); setEditVenda(undefined); },
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, data }: any) => db.updateVenda(id, data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/vendas"] }); toast({ title: "Atualizado!" }); setEditVenda(undefined); },
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => db.deleteVenda(id),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/vendas"] }); toast({ title: "Removido" }); },
  });
